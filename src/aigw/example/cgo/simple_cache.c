...
OuterEntry* get_or_create_outer_entry(const char* model_name)
{
    ...
    if (first_free) {
        strncpy(first_free->model_name, model_name, MAX_MODEL_NAME_LEN - 1);
        first_free->model_name[MAX_MODEL_NAME_LEN - 1] = '\0';
        memset(first_free->inner_map, 0, sizeof(first_free->inner_map)); // state=0
        first_free->in_use = 1;
        pthread_rwlock_init(&first_free->lock, NULL);
...
static aigw_error_t simple_cache_hash_get_all(const char *model_name, key_value_array_t *out_array)
{
    ...
    if (count > 0) {
        out_array->pairs = (key_value_pair_t*)malloc(count * sizeof(key_value_pair_t));
        if (!out_array->pairs) {
    ...
    int idx = 0;
    for (int i = 0; i < HASH_MAP_SIZE; i++) {
        InnerEntry* entry = &outer->inner_map[i];
        if (entry->state == STATE_OCCUPIED) {
            key_value_pair_t *kv = &out_array->pairs[idx++];
 ...
 */
static void simple_cache_cleanup()
{
    pthread_mutex_lock(&global_mutex);
    for (int i = 0; i < HASH_MAP_SIZE; i++) {
...
static aigw_error_t test_simple_cache_query_and_delete(const char *model_name)
{
    ...
    if (result.pairs) free(result.pairs);
    if (result2.pairs) free(result2.pairs);
