...
#define QBUF_POOL_EXPANSION_THRESHOLD_MEM_CAP (32ULL * 1024 * 1024) // per-SC cap of expand threshold (in bytes)
...
#define QBUF_BYTES_PER_MB (1024.0 * 1024.0)  // bytes per MB for human-readable sizes
...
typedef struct qbuf_pool {
    ...
    // per-SC historical max outstanding (alloc - free high-water mark)
    volatile uint64_t outstanding_max[UMQ_QBUF_SIZE_CLASS_MAX];
    ...
    volatile uint64_t nodata_outstanding_max;
} qbuf_pool_t;
...
static const char *qbuf_lc_labels[QBUF_LC_PATHS] = {
...
__attribute__((noinline, cold)) static void qbuf_dbg_record_alloc_nodata(uint64_t _t0)
{
    ...
    if (_dt > g_dbg_stats.alloc_lc_ns_max[_lc]) {
        g_dbg_stats.alloc_lc_ns_max[_lc] = _dt;
    }
...
__attribute__((noinline, cold)) static void qbuf_dbg_record_alloc_data(uint64_t _t0, uint32_t sc)
{
    ...
    if (_dt > g_dbg_stats.alloc_sc_ns_max[sc]) {
        g_dbg_stats.alloc_sc_ns_max[sc] = _dt;
    }
}
...
__attribute__((noinline, cold)) static void qbuf_dbg_record_free(uint64_t _t0)
{
    ...
    if (_dt > g_dbg_stats.free_ns_max) {
        g_dbg_stats.free_ns_max = _dt;
    }
}
#endif
...
void qbuf_log_non_pool_pointer(const char *caller, void *data)
{
    ...
    /* Dump RX pool and tiny pool ranges */
...
/*
 * Dynamic expand threshold for with_data pools (without_data keeps its
 * static threshold, see umq_qbuf_exp_pool_inner_init):
 *   threshold = min((global_capacity + exp_capacity) * threshold% / 100,
 *                   32MB expressed in blocks)
 * Recomputed at init and after every expand/shrink so the async prefill
 * trigger tracks the actual pool size; the 32MB cap bounds the free
 * reserve on large pools.
 * Caller must hold exp_pool->expansion_pool_lock.
 */
static void update_with_data_expand_threshold(uint32_t sc, qbuf_expansion_pool_t *exp_pool)
{
    uint64_t total_blocks = g_qbuf_pool.per_sc_block_counts[sc] + exp_pool->exp_total_capacity;
    uint64_t threshold = total_blocks * g_qbuf_pool.expansion_threshold / 100;
    uint64_t cap_blocks = QBUF_POOL_EXPANSION_THRESHOLD_MEM_CAP / g_qbuf_pool.block_sizes[sc];

    if (cap_blocks == 0) {
        cap_blocks = 1;
    }
    if (threshold > cap_blocks) {
        threshold = cap_blocks;
    }
    __atomic_store_n(&exp_pool->trigger_expand_block_num, threshold, __ATOMIC_RELAXED);
}
 ...
 * shrink_line_out, when non-NULL, receives the admission line in blocks
 * for the SHRINK/SHRINK_SKIP logs.
 */
...
static void async_shrink_push_param(bool with_data, uint32_t sc, qbuf_expansion_pool_t *exp_pool, uint32_t slot_id)
{
    ...
    (void)pthread_cond_signal(&exp_pool->shrink_task_list.cond);
...
static void *async_shrink_global_pool_callback(void *arg)
{
    ...
    while (true) {
        ...
        (void)pthread_mutex_lock(&exp_pool->shrink_task_list.mutex);
        while (urpc_list_is_empty(&exp_pool->shrink_task_list.head) && !exp_pool->shrink_task_list.stop) {
            (void)pthread_cond_wait(&exp_pool->shrink_task_list.cond, &exp_pool->shrink_task_list.mutex);
        }
        if (exp_pool->shrink_task_list.stop) {
            (void)pthread_mutex_unlock(&exp_pool->shrink_task_list.mutex);
            break;
        }
        /* List is non-empty — drain all pending tasks. */
        (void)pthread_mutex_unlock(&exp_pool->shrink_task_list.mutex);
        ...
        while ((shrink_param = async_shrink_pop_param(exp_pool)) != NULL) {
            ...
            /* Exec-time snapshots under expansion_pool_lock: push-time state
             * may be stale after the decay window or after other tasks were
             * processed. */
            uint64_t blk_count = slot->total_block_cnt;
            uint64_t e_free_before = exp_pool->exp_total_block_num;
            uint64_t e_cap_before = exp_pool->exp_total_capacity;
            uint64_t g_free_before = shrink_param->with_data ?
                g_qbuf_pool.block_pool[shrink_param->sc].buf_cnt_with_data :
                g_qbuf_pool.block_pool[0].buf_cnt_without_data;
            uint64_t g_cap = shrink_param->with_data ? g_qbuf_pool.per_sc_block_counts[shrink_param->sc] :
            ...
            exp_pool->exp_total_block_num -= blk_count;
            exp_pool->exp_total_capacity -= blk_count;
            if (shrink_param->with_data) {
                update_with_data_expand_threshold(shrink_param->sc, exp_pool);
            }
            uint64_t e_free_after = exp_pool->exp_total_block_num;
            ...
            UMQ_VLOG_INFO(VLOG_UMQ,
                ...
                "tls_pool_blk=%llu "
                "free=%llu->%llu(g_free=%llu e_free=%llu->%llu) "
                "total=%llu->%llu(g_cap=%llu e_cap=%llu->%llu) "
                "slots_after=%u "
                ...
                (unsigned long long)shrink_line,
                (unsigned long long)trigger_expand_before,
                (unsigned long long)trigger_expand_after,
                (unsigned long long)shrink_tls_pool,
                (unsigned long long)(g_free_before + e_free_before),
                (unsigned long long)(g_free_before + e_free_after),
                (unsigned long long)g_free_before,
                (unsigned long long)e_free_before,
                (unsigned long long)e_free_after,
                (unsigned long long)(g_cap + e_cap_before),
                (unsigned long long)(g_cap + e_cap_after),
                (unsigned long long)g_cap,
                (unsigned long long)e_cap_before,
                (unsigned long long)e_cap_after,
                slots_after,
                (unsigned long long)shrink_sc_outstanding,
                shrink_outstanding_mb,
            ...
            /* Chained re-check (with_data only): a slot that became fully
             * free after the ratio check failed gets no later return event
             * (returns only touch slots with outstanding buffers) and would
             * strand. Re-scan and enqueue one eligible slot; processing it
             * re-runs this scan, and each shrink lowers the free ratio, so
             * the chain drains all candidates and self-terminates. */
            if (shrink_wd) {
                uint32_t chain_slot_id = 0;
                bool chain_found = false;
                (void)pthread_spin_lock(&exp_pool->expansion_pool_lock);
                if (exp_pool->inited) {
                    qbuf_expansion_pool_slot_t *chain_slot;
                    URPC_LIST_FOR_EACH(chain_slot, node, &exp_pool->slot_list)
                    {
                        if (chain_slot->free_block_cnt != chain_slot->total_block_cnt
                            || chain_slot->shrink_pending) {
                            continue;
                        }
                        if (with_data_shrink_admitted(shrink_sc, exp_pool, chain_slot->total_block_cnt, NULL)) {
                            chain_slot->shrink_pending = true;
                            chain_slot_id = chain_slot->slot_id;
                            chain_found = true;
                            break;
                        }
                    }
                ...
                (void)pthread_spin_unlock(&exp_pool->expansion_pool_lock);
                if (chain_found) {
                    async_shrink_push_param(true, shrink_sc, exp_pool, chain_slot_id);
                }
            }
        }
    }

    UMQ_VLOG_DEBUG(VLOG_UMQ, "%s_SHRINK_ASYNC thread exiting, total_shrink=%llu\n",
...
static void async_shrink_global_pool(bool with_data, uint32_t sc, uint32_t slot_id)
{
    ...
    async_shrink_push_param(with_data, sc, exp_pool, slot_id);
}
                                                         ...
                                                         uint32_t sc)
{
   ...
   /* Check shrink trigger inside the lock — reading slot->free_block_cnt
    ...
    uint64_t exp_free_snapshot = exp_pool->exp_total_block_num;
    uint64_t shrink_line; /* shrink admission line in blocks, reported by the SHRINK_SKIP log */
    bool shrink_cond;
    if (with_data) {
...
{
    /* Mark as not inited first so the shrink thread skips processing
     * any remaining tasks after its current decay sleep. */
    (void)pthread_spin_lock(&exp_pool->expansion_pool_lock);
    exp_pool->inited = false;
    (void)pthread_spin_unlock(&exp_pool->expansion_pool_lock);

    /* Signal the dedicated shrink thread to stop and wait for it to exit.
     * pthread_join guarantees the thread is fully dead before we proceed
     * to clean up slots and destroy locks — no lock-leak timeout needed. */
    if (exp_pool->shrink_thread_created) {
        (void)pthread_mutex_lock(&exp_pool->shrink_task_list.mutex);
        exp_pool->shrink_task_list.stop = true;
        (void)pthread_cond_signal(&exp_pool->shrink_task_list.cond);
        (void)pthread_mutex_unlock(&exp_pool->shrink_task_list.mutex);
        (void)pthread_join(exp_pool->shrink_thread, NULL);
                                        ...
                                        uint32_t sc)
{
    ...
    if (with_data) {
        ...
        if (exp_pool->expansion_block_count == 0) {
            exp_pool->expansion_block_count = QBUF_POOL_MIN_EXP_BLK_CNT;
        }
        if (g_qbuf_pool.mode == UMQ_BUF_SPLIT) {
    ...
    if (pthread_create(&exp_pool->shrink_thread, NULL, async_shrink_global_pool_callback, (void *)exp_pool) != 0) {
        UMQ_LIMIT_VLOG_ERR(VLOG_UMQ, "create shrink thread failed, errno: %d\n", errno);
    } else {
        exp_pool->shrink_thread_created = true;
    }
    return UMQ_SUCCESS;
}
...
static int init_size_class_config(const qbuf_pool_cfg_t *cfg, uint64_t max_umq_buf_pool_size)
{
    ...
    if (base < QBUF_POOL_BLOCK_SIZE_ALIGN || base > QBUF_POOL_MAX_BLOCK_SIZE ||
        base % QBUF_POOL_BLOCK_SIZE_ALIGN != 0) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (bs == 0 || bs > QBUF_POOL_MAX_BLOCK_SIZE || bs % QBUF_POOL_BLOCK_SIZE_ALIGN != 0) {
    ...
    if (max_umq_buf_pool_size < g_total_len) {
        UMQ_VLOG_ERR(VLOG_UMQ,
                     "max_umq_buf_pool_size %llu < g_total_len %llu, "
                     "expansion_mem_size_max would underflow\n",
    ...
    if (cfg->tls_expand_qbuf_pool_depth != 0 && tls_qbuf_pool_depth_in_range(cfg->tls_expand_qbuf_pool_depth)) {
        ...
        g_qbuf_pool.tls_expand_qbuf_pool_depth = g_qbuf_pool.tls_qbuf_pool_depth / QBUF_POOL_TLS_EXPAND_DEPTH_DIVISOR;
...
static void init_split_mode_layout(const qbuf_pool_cfg_t *cfg, uint32_t count)
{
    ...
    for (uint32_t k = 0; k < order_cnt; k++) {
        ...
        update_with_data_expand_threshold(sc, &g_qbuf_pool.exp_pool_with_data[sc]);
        header_cur += blk_num * sizeof(umq_buf_t);
        UMQ_VLOG_INFO(VLOG_UMQ, "qbuf pool SPLIT: sc=%u blk_size=%u num=%lu\n", sc, blk_size,
                      (unsigned long)blk_num);
    }
    // Lazy SCs: zero block pool counts (no data region allocated). Dynamic
    // threshold starts at 0 (G=0, E=0) and turns positive on first expansion.
    for (uint32_t sc = 0; sc < count; sc++) {
        if (g_qbuf_pool.per_sc_block_counts[sc] != 0) {
            continue;
        }
    ...
    for (uint32_t _sc = 0; _sc < g_qbuf_pool.size_class_count; _sc++) {
        UMQ_VLOG_INFO(VLOG_UMQ, "  sc=%u: blk_size=%u global_with_data=%llu tls_depth=%llu trigger_expand=%llu\n", _sc,
                      ...
                      (unsigned long long)__atomic_load_n(
                          &g_qbuf_pool.exp_pool_with_data[_sc].trigger_expand_block_num, __ATOMIC_RELAXED));
    }
                                                              ...
                                                              bool with_data, uint32_t sc)
{
    ...
    while (fetch_count < need_batch) {
        UMQ_VLOG_DEBUG(VLOG_UMQ, "FETCH: %s sc=%u need_batch=%u fetched=%u global=%llu exp_total=%llu mpool=%u\n",
                       ...
                       (unsigned long long)(with_data ? g_qbuf_pool.block_pool[sc].buf_cnt_with_data
                                                     : g_qbuf_pool.block_pool[0].buf_cnt_without_data),
                       ...
                       QBUF_LIST_FIRST(local_head) ? QBUF_LIST_FIRST(local_head)->mempool_id
                                                   : QBUF_POOL_MEMPOOL_ID_INVALID);
        ret = fetch_from_global(&g_qbuf_pool.block_pool[sc], local_pool, with_data, sc, batch_cnt);
        UMQ_VLOG_DEBUG(VLOG_UMQ, "FETCH result: ret=%d mpool=%u\n", ret,
                       (ret > 0 && QBUF_LIST_FIRST(local_head)) ? QBUF_LIST_FIRST(local_head)->mempool_id
                                                                : QBUF_POOL_MEMPOOL_ID_INVALID);
        if (ret > 0 && !with_data) {
            if (qbuf_debug_on())
    ...
    if (with_data) {
        ...
        if (actual_cnt > cur_cap) {
            ...
            if (g_total + delta > g_qbuf_pool.per_sc_tls_qbuf_pool_depth[sc]) {
                delta = (g_total >= g_qbuf_pool.per_sc_tls_qbuf_pool_depth[sc])
                        ? 0
                        : (g_qbuf_pool.per_sc_tls_qbuf_pool_depth[sc] - g_total);
            }
            if (delta > 0) {
...
ROLLBACK:
    UMQ_VLOG_DEBUG(VLOG_UMQ, "FETCH_ROLLBACK: %s sc=%u fetched=%u need=%u mpool=%u\n",
                   with_data ? "data" : "nodata", sc, fetch_count, need_batch,
                   local_head_before ? local_head_before->mempool_id : QBUF_POOL_MEMPOOL_ID_INVALID);
...
static int expand_global_pool_impl(bool with_data, uint32_t sc, bool already_locked, uint64_t g_buf_cnt)
{
    ...
    /* g_cap is static after init: per-SC with-data capacity, or the fixed
     * without-data header count. */
    uint64_t g_cap = with_data ? g_qbuf_pool.per_sc_block_counts[sc] : (uint64_t)QBUF_POOL_INITIAL_NODATA_BUF_CNT;
    ...
    if (with_data) {
        update_with_data_expand_threshold(sc, exp_pool);
    }
    ...
    UMQ_VLOG_INFO(VLOG_UMQ,
        ...
        (unsigned long long)(g_cap + exp_pool_total_before),
        (unsigned long long)(g_cap + exp_pool_total_after),
        (unsigned long long)g_cap,
...
void async_expand_global_pool(bool with_data, uint32_t sc, uint64_t g_buf_cnt)
{
    ...
    qbuf_expansion_pool_t *exp_pool = with_data ? &g_qbuf_pool.exp_pool_with_data[sc] :
                                                  &g_qbuf_pool.exp_pool_without_date;
    uint64_t trigger_expand = __atomic_load_n(&exp_pool->trigger_expand_block_num, __ATOMIC_RELAXED);
    uint64_t exp_free = exp_pool->exp_total_block_num;
    if (g_buf_cnt + exp_free >= trigger_expand) {
        return;
    }
                                    ...
                                    uint64_t *local_buf_cnt)
{
    ...
    UMQ_VLOG_DEBUG(VLOG_UMQ, "FETCH_EXPANSION: %s sc=%u need=%u mpool_first=%u\n",
            ...
            (urpc_list_is_empty(&exp_pool->slot_list) ? QBUF_POOL_MEMPOOL_ID_INVALID :
    ...
    UMQ_VLOG_DEBUG(VLOG_UMQ, "FETCH_EXPANSION result: %s sc=%u got=%u/%u mpool=%u\n",
                   with_data ? "data" : "nodata", sc, count, need,
                   (count > 0 && QBUF_LIST_FIRST(local_head)) ? QBUF_LIST_FIRST(local_head)->mempool_id
                                                              : QBUF_POOL_MEMPOOL_ID_INVALID);
    return count;
}
...
int umq_normal_qbuf_alloc(uint32_t request_size, uint32_t num, umq_alloc_option_t *option, umq_buf_list_t *list)
{
    ...
    if (request_size == 0) {
        ...
        UMQ_VLOG_DEBUG(VLOG_UMQ, "ALLOC nodata: need=%u tls=%llu %s mpool=%u\n", num,
                       ...
                       QBUF_LIST_FIRST(&local_pool->head_without_data)
                           ? QBUF_LIST_FIRST(&local_pool->head_without_data)->mempool_id
                           : QBUF_POOL_MEMPOOL_ID_INVALID);
    ...
    if (sc >= g_qbuf_pool.size_class_count) {
        ...
        UMQ_LIMIT_VLOG_ERR(VLOG_UMQ,
                           ...
                           g_qbuf_pool.size_class_count > 0
                               ? g_qbuf_pool.block_sizes[g_qbuf_pool.size_class_count - 1] : 0);
        return -UMQ_ERR_ENOMEM;
    }
    uint32_t blk_size = g_qbuf_pool.block_sizes[sc];
    ...
    UMQ_VLOG_DEBUG(VLOG_UMQ, "ALLOC data: req=%u num=%u sc=%u blk=%u need=%u tls=%u %s mpool=%u\n",
                   request_size, num, sc, blk_size, needed, buf_cnt,
                   buf_cnt >= needed ? "TLS_HIT" : "TLS_MISS",
                   QBUF_LIST_FIRST(&local_pool->head_with_data[sc])
                       ? QBUF_LIST_FIRST(&local_pool->head_with_data[sc])->mempool_id
                       : QBUF_POOL_MEMPOOL_ID_INVALID);
    if (buf_cnt >= needed) {
        if (qbuf_debug_on()) {
    ...
    if (qbuf_debug_on()) {
        qbuf_dbg_record_alloc_data(_t0, sc);
    }
    return UMQ_SUCCESS;
}
...
void umq_qbuf_free(umq_buf_list_t *list)
{
    ...
    while (cur != NULL) {
        ...
        total_cnt++;
        if (cur->rx_fallback) {
            __atomic_sub_fetch(&g_rx_fallback_outstanding, 1, __ATOMIC_RELAXED);
        }
        cur = next;
    }
    QBUF_LIST_FIRST(list) = NULL;
    ...
    for (uint32_t sc = 0; sc < g_qbuf_pool.size_class_count; sc++) {
        ...
        uint64_t cap_cnt = g_qbuf_pool.disable_scale_cap
        ...
        if (in_main_pool) {
            ...
            if (sc < UMQ_QBUF_SIZE_CLASS_MAX) {
                ...
                if (id < g_qbuf_pool.per_sc_block_counts[sc]) {
                    umq_buf_t *candidate =
                        (umq_buf_t *)(uintptr_t)(g_qbuf_pool.header_region_start[sc] + id * sizeof(umq_buf_t));
...
int umq_qbuf_pool_info_get(umq_qbuf_pool_stats_t *qbuf_pool_stats)
{
    ...
    qbuf_pool_stats->alloc_stats.nodata_outstanding_max =
    ...
    qbuf_pool_stats->alloc_stats.rx_pool_outstanding_max = umq_rx_qbuf_pool_outstanding_max_get();
    ...
    for (uint32_t sc = 0; sc < g_qbuf_pool.size_class_count; sc++) {
        ...
        qbuf_pool_stats->alloc_stats.sc_alloc_count[sc] =
            __atomic_load_n(&g_qbuf_pool.alloc_count[sc], __ATOMIC_RELAXED);
        qbuf_pool_stats->alloc_stats.sc_free_count[sc]  =
            __atomic_load_n(&g_qbuf_pool.free_count[sc], __ATOMIC_RELAXED);
        qbuf_pool_stats->alloc_stats.sc_outstanding_max[sc] =
            __atomic_load_n(&g_qbuf_pool.outstanding_max[sc], __ATOMIC_RELAXED);
        if (g_qbuf_pool.disable_scale_cap) {
            continue;
        }
        ...
        sci->trigger_expand = __atomic_load_n(&e->trigger_expand_block_num, __ATOMIC_RELAXED);
    ...
    if (!g_tls_dtors_running) {
        ...
        URPC_LIST_FOR_EACH(pool_iter, tls_node, &g_tls_register_head)
        {
            ...
            umq_local_qbuf_pool_stats_t *s =
                &qbuf_pool_stats->local_qbuf_pool_stats[qbuf_pool_stats->local_qbuf_pool_num];
