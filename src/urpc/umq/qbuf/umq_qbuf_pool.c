...
static inline uint32_t umq_qbuf_pool_tls_depth(void)
{
    return (g_umq_qbuf_size_pow_small == UMQ_QBUF_SIZE_POW_4K) ?
        (QBUF_POOL_TLS_QBUF_DEPTH << 1) : QBUF_POOL_TLS_QBUF_DEPTH;
}
...
static ALWAYS_INLINE void release_thread_cache(uint64_t id)
{
    ...
    __atomic_fetch_sub(&g_total_local_cap_with_data, local_pool->capacity_with_data, __ATOMIC_ACQ_REL);
    __atomic_fetch_sub(&g_total_local_cap_without_data, local_pool->capacity_without_data, __ATOMIC_ACQ_REL);
...
static ALWAYS_INLINE void thread_cache_self_shrink(bool with_data)
{
    ...
    *cap_ptr = (*cap_ptr) - shrink;
    __atomic_fetch_sub(g_total, shrink, __ATOMIC_ACQ_REL);
