 ...
 * view (where only block_sizes[0] was exposed and per-sc buf counts were SUM-ed).
 * Valid indices: [0..sc_count-1]. For single-level pools only sc_info[0] is filled. */
typedef struct umq_qbuf_sc_info {
    uint32_t blk_size;             // g_qbuf_pool.block_sizes[sc]
    uint64_t data_region_start;    // g_qbuf_pool.data_region_start[sc] cast to uint64_t
...
typedef struct umq_qbuf_pool_info {
    ...
    /* multi-level size_class breakdown (new). For single-level pools sc_count=1 and
     * only sc_info[0] is filled; per-sc fields are the raw per-sc values (NOT summed).
     * The legacy single-level fields above (block_size/total_block_num/available_mem)
     * are kept for existing consumers and reflect sc[0] or summed totals. */
    umq_qbuf_pool_config_t config;                           // multi-level pool configuration
    umq_qbuf_sc_info_t sc_info[UMQ_DFX_QBUF_SIZE_CLASS_MAX]; // per-sc raw values
    uint32_t sc_count;                                       // number of valid sc_info[] entries
    uint32_t exp_slot_used_count;                            // non-NULL count in g_exp_slot_table (766 entries)
