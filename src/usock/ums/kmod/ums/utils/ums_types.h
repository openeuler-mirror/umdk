...
struct ums_connection {
	...
	struct ums_token_xchg token_xchg; /* token exchange completion and result */
	...
	u8 initiator_id[UMS_SYSTEMID_LEN]; /* client ums system ID, used with clc_session_id
										...
										*/
	struct ubcore_token peer_seg_token_value;
	/*
	 ...
	 * unregister_clc_session or netlink callback completion (ums_conn_nl_put).
