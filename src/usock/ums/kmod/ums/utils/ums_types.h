...
struct ums_token_xchg {
	struct completion done;
	int result;
};

struct ums_token_xchg_ctx {
	u32 clc_session_id; /* CLC session ID generated by ums client,
			    		 * shared by both ends of the connection
			    		 */
	u8 initiator_id[UMS_SYSTEMID_LEN]; /* client ums system ID, used with clc_session_id
										* as composite key for netlink lookup
										*/
	struct ums_token_xchg xchg; /* token exchange completion and result */
	struct ubcore_token jetty_token;
	struct ubcore_token seg_token;
	struct ubcore_token peer_seg_token;
	struct ubcore_token peer_jetty_token;

	/*
	 * Prevents use-after-free when netlink callback accesses conn after
	 * the CLC handshake timeout path has started conn cleanup.
	 * Lifecycle: set to 1 on register_clc_session, decremented by
	 * unregister_clc_session or netlink callback completion (ums_token_ctx_nl_put).
	 * ums_conn_free waits for nl_refcnt == 0 before freeing conn resources.
	 */
	refcount_t nl_refcnt;
	wait_queue_head_t nl_free_wait; /* wakeup when nl_refcnt == 0 */
};
