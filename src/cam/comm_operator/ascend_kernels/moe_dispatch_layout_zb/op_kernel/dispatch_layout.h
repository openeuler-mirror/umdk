...
public:
    ...
    __aicore__ inline void Init(GM_ADDR topkIdx, GM_ADDR numTokensPerRank, GM_ADDR numTokensPerExpert,
        GM_ADDR isTokenInRank, GM_ADDR notifySendData, GM_ADDR sendTokenIdx, GM_ADDR workspace, TPipe *pipe,
        const DispatchLayoutTilingData *tilingData)
    {
        ...
        if (coreIdx_ < restNum) {
            topkIdxOffset = coreIdx_ * tempTokens_ * numTopk_ * sizeof(int32_t);
            isTokenOffset = coreIdx_ * tempTokens_ * numRanks_ * sizeof(T);
        } else {
            topkIdxOffset = (restNum + coreIdx_ * tempTokens_) * numTopk_ * sizeof(int32_t);
            isTokenOffset = (restNum + coreIdx_ * tempTokens_) * numRanks_ * sizeof(T);
        }
        tempExpertGM_.SetGlobalBuffer((__gm__ T *)notifySendData);
        topkIdxGM_.SetGlobalBuffer((__gm__ int32_t *)(topkIdx + topkIdxOffset));
        ...
        // send_token_idx is also int32; same byte offset as topk_idx.
        sendTokenIdxGM_.SetGlobalBuffer((__gm__ T *)(sendTokenIdx + topkIdxOffset));
    }
    ...
    __aicore__ inline uint32_t CalcTokensPerRound()
    {
        ...
        // topkIdxBuf: numTopk_ * sizeof(int32_t) per token
        // isTokenInRankBuf: numRanks_ * sizeof(T) per token
        // sendTokenIdxBuf: numTopk_ * sizeof(T) per token
        uint32_t perTokenSize = Ceil(numTopk_ * sizeof(int32_t), UB_32_ALIGN) * UB_32_ALIGN +
    ...
    __aicore__ inline void CountTokensInRound(uint32_t roundStart, uint32_t roundTokens, int expertsPerRank)
    {
        ...
        LocalTensor<int32_t> topkIdxLocal = topkIdxBuf_.AllocTensor<int32_t>();
        ...
        for (uint32_t i = 0; i < roundTokens; ++i) {
            ...
            for (uint32_t j = 0; j < numTopk_; ++j) {
                int32_t expertIdx = topkIdxLocal.GetValue(i * numTopk_ + j);
    ...
    __aicore__ inline void CalcSendTokenIdxInRound(uint32_t roundStart, uint32_t roundTokens)
    {
        ...
        tpipe_->InitBuffer(topkIdxBuf_, topkLen);
        tpipe_->InitBuffer(numTokensPerExpertBuf_, numTokensPerExpert32AlignIntLen_);
        tpipe_->InitBuffer(sendTokenIdxBuf_, sendIdxLen);
        LocalTensor<int32_t> topkIdxLocal = topkIdxBuf_.AllocTensor<int32_t>();
        const DataCopyExtParams topkCopyParams{1U, topkLen, 0U, 0U, 0U};
        const DataCopyPadExtParams<int32_t> topkPadParams{false, 0U, 0U, 0U};
        DataCopyPad(topkIdxLocal, topkIdxGM_[roundStart * numTopk_], topkCopyParams, topkPadParams);
        ...
        SyncFunc<AscendC::HardEvent::MTE2_S>();
        LocalTensor<T> sendIdxLocal = sendTokenIdxBuf_.AllocTensor<T>();
        for (uint32_t i = 0; i < roundTokens; ++i) {
            for (uint32_t j = 0; j < numTopk_; ++j) {
                int32_t expertIdx = topkIdxLocal.GetValue(i * numTopk_ + j);
        ...
        DataCopyPad(sendTokenIdxGM_[roundStart * numTopk_], sendIdxLocal, sendIdxCopyParams);
        PipeBarrier<PIPE_MTE3>();
    }
