...
namespace optiling {
    ...
    static ge::graphStatus CheckData(const char* nodeName, const E2ACheckParams& params)
    {
        OP_TILING_CHECK(params.rank < 0 || params.rank >= params.expertRankSize + params.attentionRankSize,
            OP_LOGE(nodeName, "CAM E2A PARAMETER INVALID: rank must >= 0 and < expertRankSize + attentionRankSize"),
            return ge::GRAPH_FAILED);

        OP_TILING_CHECK(params.batchSize <= 0 || params.batchSize > MAX_BATCH_SIZE,
            OP_LOGE(nodeName, "CAM E2A PARAMETER INVALID: batchSize must >= 0 and <= 1024"),
            return ge::GRAPH_FAILED);

        OP_TILING_CHECK(params.hiddenSize < MIN_HIDDEN_SIZE || params.hiddenSize > MAX_HIDDEN_SIZE ||
                        params.hiddenSize % HIDDEN_SIZE_ALIGN != 0,
            OP_LOGE(nodeName, "CAM E2A PARAMETER INVALID: hiddenSize must >= 1024 and <= 7168 and be divisible by 256"),
            return ge::GRAPH_FAILED);

        OP_TILING_CHECK(params.topk <= 0 || params.topk > MAX_TOPK,
            OP_LOGE(nodeName, "CAM E2A PARAMETER INVALID: topk must >= 1 and <= 8"),
            return ge::GRAPH_FAILED);

        OP_TILING_CHECK(params.expertRankSize <= 0 || params.expertRankSize > params.attentionRankSize,
            OP_LOGE(nodeName, "CAM E2A PARAMETER INVALID: expertRankSize must > 0 and <= attentionRankSize"),
            return ge::GRAPH_FAILED);

        OP_TILING_CHECK(params.aivAlgNum < AIV_ALG_NUM_MIN || params.aivAlgNum > AIV_ALG_NUM_MAX,
            OP_LOGE(nodeName, "CAM E2A PARAMETER INVALID: aivAlgNum must >= 4 and <= 48"),
            return ge::GRAPH_FAILED);
    ...
    static ge::graphStatus TilingFunc(gert::TilingContext* context)
    {
        ...
        E2ACheckParams checkParams{rank, batchSize, hiddenSize, topk, expertRankSize,
