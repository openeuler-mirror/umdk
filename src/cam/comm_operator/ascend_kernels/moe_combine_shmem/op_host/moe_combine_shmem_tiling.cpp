...
constexpr uint32_t ATTR_WINDOW_SIZE_INDEX = 13;
                                               ...
                                               uint32_t &commQuantMode)
{
    ...
    tilingData.moeDistributeCombineInfo.totalWinSize = static_cast<uint64_t>(*windowSizePtr);
...
static bool CheckTensorDim(const gert::TilingContext &context, const char *nodeName)
{
    ...
    // x_active_mask is currently not supported as input
...
static bool CheckTensorFormat(const gert::TilingContext &context, const char *nodeName)
{
    ...
    OPS_ERR_IF(
            static_cast<ge::Format>(
                ge::GetPrimaryFormat(epSendCountsDesc->GetStorageFormat())) == ge::FORMAT_FRACTAL_NZ,
    ...
    OPS_ERR_IF(
            static_cast<ge::Format>(
                ge::GetPrimaryFormat(tpSendCountsDesc->GetStorageFormat())) == ge::FORMAT_FRACTAL_NZ,
    ...
    OPS_ERR_IF(
            static_cast<ge::Format>(
                ge::GetPrimaryFormat(expertScalesDesc->GetStorageFormat())) == ge::FORMAT_FRACTAL_NZ,
...
{
    // Validate expertIds input dim1 and set k, bs already validated
    const gert::StorageShape *expertIdsStorageShape = context.GetInputShape(EXPERT_IDS_INDEX);
    OPS_ERR_IF(expertIdsStorageShape == nullptr, OPS_LOG_E(nodeName, "expertIds is null."), return false);
    ...
    if (isShared) {  // This card is a shared expert
        A = globalBs / sharedExpertRankNum;
    } else {  // This card is a moe expert
        A = globalBs * std::min(static_cast<int64_t>(localExpertNum), expertIdsDim1);
    }

    // Validate expandX dimensions and set h
    ...
    // Validate expandIdx dimensions
    const gert::StorageShape *expandIdxStorageShape = context.GetInputShape(EXPAND_IDX_INDEX);
    OPS_ERR_IF(expandIdxStorageShape == nullptr,
        OPS_LOG_E(nodeName, "expandIdxStorageShape is null"),
        return false);
    ...
    // Validate epSendCount and tpSendCount dimensions
    ...
    // Validate expertScales dimensions
    const gert::StorageShape *expertScalesStorageShape = context.GetInputShape(EXPERT_SCALES_INDEX);
                       ...
                       uint32_t &localMoeExpertNum)
{
    ...
    // Check if ep can evenly divide shared experts
    ...
    // Check if epWorldSize is a multiple of 8
    ...
    // Validate expertIds input dim0 and set bs
    const gert::StorageShape *expertIdsStorageShape = context.GetInputShape(EXPERT_IDS_INDEX);
...
static ge::graphStatus MoeDistributeCombineA3TilingFuncImpl(gert::TilingContext &context)
{
    ...
    if (epRankId >= sharedExpertRankNum) {  // This card is a moe expert
        isShared = false;
    }
    ...
    const uint64_t maxWindowSize = tilingData->moeDistributeCombineInfo.totalWinSize;
    ...
    OPS_ERR_IF(actualWinSize > maxWindowSize,
        OPS_LOG_E(nodeName,
            "actualWinSize is too large, maxBs = %lu, h = %lu, epWorldSize = %lu, "
            "localMoeExpertNum = %u, actualWinSize = %luMB, maxWindowSize = %luMB.",
                            maxBs, h, epWorldSize, localMoeExpertNum, actualWinSize / MB_SIZE + 1UL,
