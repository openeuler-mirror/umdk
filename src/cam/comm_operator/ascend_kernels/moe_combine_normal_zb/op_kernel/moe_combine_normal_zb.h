 ...
 * Description: MoeCombineNormalZb operator kernel function header file
 * Create: 2026-06-10
 * Note:
 * History: 2026-06-10 create MoeCombineNormalZb operator kernel function header file
 */
#ifndef MOE_COMBINE_NORMAL_ZB_H
#define MOE_COMBINE_NORMAL_ZB_H

#include "zb_api.h"
#include "kernel_operator.h"
#include "kernel_tiling/kernel_tiling.h"
#include "moe_combine_normal_zb_tiling.h"
#include "zb_sync_flag.h"

namespace MoeCombineNormalZbImpl {
...
public:
    ...
    __aicore__ inline MoeCombineNormalZb(){};
    __aicore__ inline void Init(GM_ADDR recvX, GM_ADDR epRecvCount, GM_ADDR topkWeights, GM_ADDR topkIdx,
        ...
        const MoeCombineNormalZbTilingData *tilingData);
...
private:
    ...
    __aicore__ inline void InitTilingData(const MoeCombineNormalZbTilingData *tilingData);
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeCombineNormalZb<TemplateMC2TypeFunc>::InitGlobalBuffer(
    ...
    GM_ADDR sendCostStatsOut)
{
    ...
    epRecvCountGT_.SetGlobalBuffer((__gm__ int32_t *)epRecvCount);  // allRecvCount: num_ranks * num_experts
...
MoeCombineNormalZb<TemplateMC2TypeFunc>::InitTilingData(const MoeCombineNormalZbTilingData *tilingData)
{
    ...
    metaDataGvaGM_ = (GM_ADDR)tilingData->commMetaPtr;
}

template <TemplateMC2TypeClass>
__aicore__ inline void MoeCombineNormalZb<TemplateMC2TypeFunc>::InitBuffLen()
{
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeCombineNormalZb<TemplateMC2TypeFunc>::Init(
    GM_ADDR recvX, GM_ADDR epRecvCount, GM_ADDR topkWeights, GM_ADDR topkIdx, GM_ADDR sendTokenIdx, GM_ADDR XOut,
    GM_ADDR sendCostStatsOut, GM_ADDR workspaceGM, TPipe *pipe, const MoeCombineNormalZbTilingData *tilingData)
{
    ...
    // Init ZbSyncFlag — per-core granularity (slotsPerRank = aivNum)
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeCombineNormalZb<TemplateMC2TypeFunc>::ReadBufferAndWeightedSum(uint32_t tokenIndex,
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeCombineNormalZb<TemplateMC2TypeFunc>::ReadBufferFromRemote()
{
    if (axisBS_ == 0U) {
        return;
    }
    ...
    // moeExpertNum max 512; tensor size = 64*512*4 = 128KB
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeCombineNormalZb<TemplateMC2TypeFunc>::Process()
{
    if ASCEND_IS_AIV {  // all AIV cores
