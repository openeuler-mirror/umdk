 ...
 * Description: MoeCombineNormalZb tiling function implementation file
 * Create: 2026-06-10
 * Note:
 * History: 2026-06-10 create MoeCombineNormalZb tiling function implementation file
 */
#include <queue>
#include <vector>
#include <dlfcn.h>
...
#include "../op_kernel/moe_combine_normal_zb_tiling.h"
...
constexpr uint32_t ATTR_COMM_META_PTR_INDEX = 0;
...
// A3-specific
static void PrintTilingDataInfo(const char *nodeName, MoeCombineNormalZbTilingData &tilingData)
{
...
static ge::graphStatus GetAttrAndSetTilingData(gert::TilingContext *context,
    MoeCombineNormalZbTilingData &tilingData, const char *nodeName)
{
    ...
    // Null checks
    OP_TILING_CHECK(commMetaPtrPtr == nullptr, OP_LOGE(nodeName, "commMetaPtr is null."), return ge::GRAPH_FAILED);
...
static bool CheckTensorShape(gert::TilingContext *context, MoeCombineNormalZbTilingData &tilingData,
    const char *nodeName, uint32_t localExpertNum)
{
    ...
    // Validate epRecvCount and tpRecvCount dims
...
static bool CheckAttrs(gert::TilingContext *context, MoeCombineNormalZbTilingData &tilingData,
    const char *nodeName, uint32_t &localMoeExpertNum)
{
    ...
    // Validate topkWeights dim0 and set bs
    const gert::StorageShape *topkWeightsStorageShape = context->GetInputShape(TOPK_WEIGHTS_INDEX);
    int64_t topkWeightsDim0 = topkWeightsStorageShape->GetStorageShape().GetDim(0);
...
static ge::graphStatus TilingCheckMoeCombineNormalZb(gert::TilingContext *context, const char *nodeName,
...
static ge::graphStatus MoeCombineNormalZbA3TilingFuncImpl(gert::TilingContext *context)
{
    const char *nodeName = context->GetNodeName();
    OP_LOGD(nodeName, "Enter MoeCombineNormalZb Tiling func");
    MoeCombineNormalZbTilingData *tilingData = context->GetTilingData<MoeCombineNormalZbTilingData>();
    ...
    OP_TILING_CHECK(TilingCheckMoeCombineNormalZb(context, nodeName, isEnableDiagnose) != ge::GRAPH_SUCCESS,
    ...
    // Combine data region: token base address 512-byte aligned
...
static ge::graphStatus MoeCombineNormalZbTilingFunc(gert::TilingContext *context)
{
    ...
    // Reject recvX when data type is DT_INT32
    OP_TILING_CHECK((recvXDesc->GetDataType() == ge::DT_INT32),
        OP_LOGE(nodeName, "recvX dataType is invalid, dataType should be bf16 or float16, but is "),
        return ge::GRAPH_FAILED);

    ge::graphStatus ret = MoeCombineNormalZbA3TilingFuncImpl(context);
    return ret;
}

struct MoeCombineNormalZbCompileInfo {};
ge::graphStatus TilingParseForMoeCombineNormalZb(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(MoeCombineNormalZb)
    .Tiling(MoeCombineNormalZbTilingFunc)
    .TilingParse<MoeCombineNormalZbCompileInfo>(TilingParseForMoeCombineNormalZb);
}  // namespace optiling
