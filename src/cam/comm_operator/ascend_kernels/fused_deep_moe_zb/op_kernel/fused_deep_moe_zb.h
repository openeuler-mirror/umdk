 ...
 * Description: FusedDeepMoeZb operator kernel function header file, for a3
 * Create: 2025-07-19
 * Note:
 * History: 2025-07-19 create FusedDeepMoeZb operator kernel function header file, for a3
 */
#ifndef FUSED_DEEP_MOE_ZB_H
#define FUSED_DEEP_MOE_ZB_H
...
#include "fused_deep_moe_zb/gemm/kernel/grouped_matmul_slice_m_per_token_dequant_multistage_workspace.h"
#include "catlass/gemm/gemm_type.hpp"
#include "fused_deep_moe_zb/epilogue/dispatch_policy.h"
#include "fused_deep_moe_zb/gemm/dispatch_policy.h"
#include "fused_deep_moe_zb/epilogue/block/block_epilogue.h"
#include "fused_deep_moe_zb/gemm/block/block_mmad.h"
#include "fused_deep_moe_zb/gemm/kernel/grouped_matmul_slice_m_per_token_dequant_swiglu_quant_multistage_workspace.h"

#include "fused_deep_moe_zb/raw_distributed/cam_moe_distribute_combine.h"

#include "fused_deep_moe_zb_tiling.h"
#include "fused_deep_moe_zb_base.h"
...
CATLASS_DEVICE void GmmDeqSwigluQuant(GemmCoord problemShape, uint32_t groupCount, GM_ADDR gmGroupList, GM_ADDR gmA,
                                  ...
                                  const FusedDeepMoeZbInfo &disGmmDeqSwigluQuantGmmDeqComInfo,
...
class FusedDeepMoeZb {
public:
    __aicore__ inline FusedDeepMoeZb(){};
    __aicore__ inline void Init(
        ...
        // system
        GM_ADDR workspaceGM, AscendC::TPipe *pipe, const FusedDeepMoeZbTilingData *tilingData);
...
private:
    ...
    const FusedDeepMoeZbTilingData *tilingData_;
    AscendC::TBuf<> roundStatusBuf_;
};

template <TemplateMC2TypeClass>
__aicore__ inline GM_ADDR FusedDeepMoeZb<TemplateMC2TypeFunc>::GetRoundStateAddrByRankId(
...
template <TemplateMC2TypeClass>
__aicore__ inline float FusedDeepMoeZb<TemplateMC2TypeFunc>::SetRoundStatus(GM_ADDR gmRoundInfo)
{
...
template <TemplateMC2TypeClass>
__aicore__ inline void FusedDeepMoeZb<TemplateMC2TypeFunc>::WaitRoundStatus(
...
template <TemplateMC2TypeClass>
__aicore__ inline void FusedDeepMoeZb<TemplateMC2TypeFunc>::Init(
    // input
    GM_ADDR x, GM_ADDR expert_ids, GM_ADDR gmm1_weight, GM_ADDR gmm1_weight_scale,
    GM_ADDR gmm2_weight, GM_ADDR gmm2_weight_scale, GM_ADDR expert_scales,
    GM_ADDR share_gmm1_weight, GM_ADDR share_gmm1_weight_scale,
...
template <TemplateMC2TypeClass>
__aicore__ inline void FusedDeepMoeZb<TemplateMC2TypeFunc>::Process()
{
    ...
    // 布局：[0, bs_*shareGmm1OutputDim_) 共享专家 C（所有 bs 个 token，M 翻倍）
    //       [bs_*shareGmm1OutputDim_, ...) 路由专家 C，按轮复用（roundRecvTokenNum 个 token，M 翻倍）
    GM_ADDR gmCVSwap = nullptr;
    if constexpr (EXEC_FLAG & EXEC_FLAG_W4A8) {
        gmCVSwap = workspaceGM_ + workspaceOffset;
        int64_t gmm1CSize = (static_cast<int64_t>(bs_) * shareGmm1OutputDim_ +
                             static_cast<int64_t>(maxTokenNum_) * gmm1OutputDim_) * sizeof(int32_t) * CONSTANT_TWO;
