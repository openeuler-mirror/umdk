 ...
 * Description: FusedDeepMoeZb tiling function implementation file
 * Create: 2025-07-19
 * Note:
 * History: 2025-07-19 create FusedDeepMoeZb tiling function implementation file
 */
#include <algorithm>
#include <cstdint>
#include <cstdio>
...
#include "../op_kernel/fused_deep_moe_zb_tiling.h"

using namespace ge;
using namespace Cam;
using namespace Util;
namespace {
constexpr const char *OPS_UTILS_LOG_SUB_MOD_NAME = "FUSED_DEEP_MOE_ZB";
...
static ge::graphStatus CheckGmm1Shape(gert::TilingContext &context, FusedDeepMoeZbTilingData &tilingData)
{
...
static ge::graphStatus CheckShareExpertShapes(gert::TilingContext &context, FusedDeepMoeZbTilingData &tilingData)
{
    const char *nodeName = context.GetNodeName();
    uint32_t h = tilingData.disGmmDeqSwigluQuantGmmDeqComInfo.h;
    ...
    const gert::StorageShape* shareGmm1ScaleStorageShape =
        context.GetOptionalInputShape(INPUT_SHARE_GMM1_WEIGHT_SCALE_INDEX);
    ...
    const gert::StorageShape* shareGmm2WeightStorageShape =
        context.GetOptionalInputShape(INPUT_SHARE_GMM2_WEIGHT_INDEX);
    ...
    if (gmm2WeightDims == TWO_DIMS) {  // [shareGmm1HLen/2, h] format
        ...
        OPS_ERR_IF(h != shareGmm2OriginShape.GetDim(2),
            OPS_LOG_E(nodeName, "shareGmm2Weight dim2 should be h(%u), but got %ld.",
                      h, shareGmm2OriginShape.GetDim(2)),
            return ge::GRAPH_FAILED);
    }

    // Check share_gmm2_weight_scale: [h] (1D) or [1, h] (2D)
    const gert::StorageShape* shareGmm2ScaleStorageShape =
        context.GetOptionalInputShape(INPUT_SHARE_GMM2_WEIGHT_SCALE_INDEX);
...
static ge::graphStatus CheckShareExpertDtypes(gert::TilingContext &context, FusedDeepMoeZbTilingData &tilingData)
{
    const char *nodeName = context.GetNodeName();
...
static ge::graphStatus CheckGmm1ScaleShape(gert::TilingContext &context,
                                           const FusedDeepMoeZbTilingData &tilingData)
{
    const char *nodeName = context.GetNodeName();
    uint32_t moeExpertNumPerRank = tilingData.disGmmDeqSwigluQuantGmmDeqComInfo.moeExpertNumPerRank;
    ...
    OPS_ERR_IF(listFlag != tilingData.disGmmDeqSwigluQuantGmmDeqComInfo.isTensorList,
        OPS_LOG_E(nodeName, "gmm1Scale listFlag does not match gmm1Weight listFlag."), return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus CheckGmm2Shape(const gert::TilingContext &context, const FusedDeepMoeZbTilingData &tilingData)
{
    const char *nodeName = context.GetNodeName();
    uint32_t moeExpertNumPerRank = tilingData.disGmmDeqSwigluQuantGmmDeqComInfo.moeExpertNumPerRank;
    uint32_t h = tilingData.disGmmDeqSwigluQuantGmmDeqComInfo.h;
...
static ge::graphStatus CheckWeightTensorList(gert::TilingContext &context,
                                             FusedDeepMoeZbTilingData &tilingData)
{
...
ge::graphStatus CheckSmoothScales(const gert::TilingContext &context, const char *nodeName,
                                  const FusedDeepMoeZbTilingData &tilingData, bool calShareExpert)
{
    ...
    OPS_ERR_IF((expertSmoothScalesTensor->GetDataType() != ge::DT_FLOAT),
                    OPS_LOG_E(nodeName,
                              "expertSmoothScales datatype is invalid, datatype should be float32, but is %d.",
    ...
    if (calShareExpert) {
        ...
        OPS_ERR_IF((shareSmoothScalesTensor->GetDataType() != ge::DT_FLOAT),
                OPS_LOG_E(nodeName,
                          "shareSmoothScales datatype is invalid, datatype should be float32, but is %d.",
...
static ge::graphStatus CheckData(const char *nodeName, FusedDeepMoeZbTilingData &tilingData)
{
    uint32_t batchSize = tilingData.disGmmDeqSwigluQuantGmmDeqComInfo.bs;
    ...
    OPS_ERR_IF(topK > SUPPORT_TOP_K, OPS_LOG_E(nodeName, "topK(k) must <= %u.", SUPPORT_TOP_K),
                    return ge::GRAPH_FAILED);
    OPS_ERR_IF(topK > tilingData.disGmmDeqSwigluQuantGmmDeqComInfo.moeExpertNum,
...
static ge::graphStatus GetAttrAndSetTilingData(const gert::TilingContext &context, const char *nodeName,
                                               FusedDeepMoeZbTilingData &tilingData, std::string &groupEp,
...
static ge::graphStatus SetRoundRecvTokenNum(const char *nodeName, FusedDeepMoeZbTilingData &tilingData,
                                            uint64_t shmemWorkspaceSize)
{
...
static void SetHcommCfg(const gert::TilingContext &context, FusedDeepMoeZbTilingData &tiling, const std::string groupEp)
{
    const char *nodeName = context.GetNodeName();
    OPS_LOG_D(nodeName, "FusedDeepMoeZb groupEp = %s", groupEp.c_str());
...
static ge::graphStatus CheckHcclBufferSize(const char *nodeName, const FusedDeepMoeZbTilingData &tilingData)
{
    uint64_t shmemWorkspacePtr = tilingData.disGmmDeqSwigluQuantGmmDeqComInfo.shmemWorkspacePtr;
    if (shmemWorkspacePtr != 0) {
        return ge::GRAPH_SUCCESS;
...
static ge::graphStatus SetWorkSpace(gert::TilingContext &context, const char *nodeName,
                                    FusedDeepMoeZbTilingData &tilingData, bool calShareExpert)
{
    ...
    // 布局：[0, batchSize*shareGmm1HLen) 共享专家 C（所有 bs 个 token，M 翻倍）
    //       [batchSize*shareGmm1HLen, ...) 路由专家 C，按轮复用（roundRecvTokenNum 个 token，M 翻倍）
    bool isW4a8 = CheckOptionalInputExist(context, INPUT_GMM1_BIAS_INDEX);
    size_t gmm1CSize = 0;
    if (isW4a8) {
        uint64_t cvSwapElems = (static_cast<uint64_t>(batchSize) * shareGmm1HLen +
                                static_cast<uint64_t>(maxTokenNum) * gmm1HLen) * CONSTANT_TWO;
        gmm1CSize = CeilUp(static_cast<size_t>(cvSwapElems * sizeof(int32_t)), GM_ALIGN_SIZE);
    }
...
static ge::graphStatus FusedDeepMoeZbTilingFuncImpl(gert::TilingContext &context)
{
    const char *nodeName = context.GetNodeName();
    OPS_ERR_IF(nodeName == nullptr, OPS_LOG_E("unKnownNodeName", "nodeName is nullptr."), return ge::GRAPH_FAILED);
    FusedDeepMoeZbTilingData *tilingData = context.GetTilingData<FusedDeepMoeZbTilingData>();
...
static ge::graphStatus FusedDeepMoeZbTilingFunc(gert::TilingContext *context)
{
    ge::graphStatus ret = FusedDeepMoeZbTilingFuncImpl(*context);
    return ret;
}

struct FusedDeepMoeZbCompileInfo {};
ge::graphStatus TilingParseForFusedDeepMoeZb(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(FusedDeepMoeZb)
    .Tiling(FusedDeepMoeZbTilingFunc)
    .TilingParse<FusedDeepMoeZbCompileInfo>(TilingParseForFusedDeepMoeZb);
}  // namespace optiling
