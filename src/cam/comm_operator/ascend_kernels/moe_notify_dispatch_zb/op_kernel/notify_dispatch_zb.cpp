...
#include "notify_dispatch_zb_tiling.h"
#include "notify_dispatch_zb.h"

#define TILING_KEY_INT_ZB 223

#define KERNEL_USE_WORKSPACE (1 * 1024 * 1024)

extern "C" __global__ __aicore__ void notify_dispatch_zb(GM_ADDR numTokensPerExpert, GM_ADDR recvData,
    GM_ADDR totalRecvTokens, GM_ADDR maxBs, GM_ADDR recvTokensPerExpert, GM_ADDR putOffset, GM_ADDR workspace,
    GM_ADDR tiling)
{
    REGISTER_TILING_DEFAULT(NotifyDispatchZbTilingData);
    GET_TILING_DATA_WITH_STRUCT(NotifyDispatchZbTilingData, tilingData, tiling);
    ...
    uint64_t commMetaPtr = tilingData.commMetaPtr;

    GM_ADDR numTokensPerExpertInput = numTokensPerExpert;
    ...
    if (TILING_KEY_IS(TILING_KEY_INT_ZB)) {
        NotifyDispatchZb<int> opKernel(rank, rankSize, extraFlag);
