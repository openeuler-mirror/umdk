 ...
 * Description: NotifyDispatchZb tiling function implementation file
 * Create: 2026-06-10
 * Note:
 * History: 2026-06-10 create NotifyDispatchZb tiling function implementation file
 */
#include <queue>
#include <vector>
#include <dlfcn.h>
...
#include "../op_kernel/notify_dispatch_zb_tiling.h"

#include "platform/platform_infos_def.h"

using namespace ge;
namespace {
constexpr uint32_t INPUT_NUM_TOKENS_PER_EXPERT_INDEX = 0;
...
constexpr uint32_t ATTR_COMM_META_PTR_INDEX = 6;
...
constexpr static int TILING_KEY_INT_ZB = 223;
constexpr static int TILING_KEY_A2_TYPE = 100;

}  // namespace

namespace optiling {
static void PrintTilingDataInfo(const char *nodeName, NotifyDispatchZbTilingData &tilingData)
{
...
static ge::graphStatus GetAttrAndSetTilingData(gert::TilingContext *context, const char *nodeName,
    NotifyDispatchZbTilingData &tilingData)
{
    ...
    OP_TILING_CHECK(commMetaPtrPtr == nullptr, OP_LOGE(nodeName, "commMetaPtrPtr is null."), return ge::GRAPH_FAILED);
...
{
    auto numTokensPerExpert = context->GetInputDesc(INPUT_NUM_TOKENS_PER_EXPERT_INDEX);
    OP_TILING_CHECK(numTokensPerExpert == nullptr, OP_LOGE(nodeName, "numTokensPerExpert is null."), return false);
    OP_TILING_CHECK(
        (numTokensPerExpert->GetDataType() != ge::DT_BF16) && (numTokensPerExpert->GetDataType() != ge::DT_FLOAT16) &&
        (numTokensPerExpert->GetDataType() != ge::DT_FLOAT) && (numTokensPerExpert->GetDataType() != ge::DT_INT32),
        OP_LOGE(
            nodeName,
            "numTokensPerExpert datatype is invalid, datatype should be bf16 or float16 or float or int, but is %d.",
...
static ge::graphStatus NotifyDispatchZbTilingFuncImpl(gert::TilingContext *context)
{
    const char *nodeName = context->GetNodeName();
    NotifyDispatchZbTilingData *tilingData = context->GetTilingData<NotifyDispatchZbTilingData>();
    OP_TILING_CHECK(tilingData == nullptr, OP_LOGE(nodeName, "tilingData is nullptr."), return ge::GRAPH_FAILED);
    OP_LOGI(nodeName, "Enter NotifyDispatchZb tiling check func.");
    ...
    int tilingKey = TILING_KEY_INT_ZB;
...
static ge::graphStatus NotifyDispatchZbTilingFunc(gert::TilingContext *context)
{
    ge::graphStatus ret = NotifyDispatchZbTilingFuncImpl(context);
    return ret;
}

struct NotifyDispatchZbCompileInfo {};
ge::graphStatus TilingParseForNotifyDispatchZb(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(NotifyDispatchZb)
    .Tiling(NotifyDispatchZbTilingFunc)
    .TilingParse<NotifyDispatchZbCompileInfo>(TilingParseForNotifyDispatchZb);
}  // namespace optiling
