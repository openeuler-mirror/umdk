...
# This sample demonstrates how to use ZbBuffer to initialize SHMEM
# and call the SHMEM version of fused_deep_moe (zb_buffer.zb_fused_deep_moe).
#
# Both W8A8 and W4A8 quantization modes are supported, controlled by the
# `--with_w4a8` flag:
#   - W8A8 (default): INT8 weights + per-channel scale, reference is computed
#     by a combination of small NPU operators (SmallOps).
#   - W4A8: INT32 packed INT4 weights + dequant bias, reference is computed
#     by a pure-PyTorch CPU simulator (TorchSimulator).
#
# The sample only exercises the ZeroBuffer path; the non-SHMEM
# torch.ops.umdk_cam_op_lib.fused_deep_moe path is intentionally not used.
#
# Create: 2026-04-22
# Note:
# History: 2026-04-22 create example file for SHMEM fused_deep_moe
# History: 2026-09-20 merge W4A8 sample into this file, keep only the
#          zb_buffer.zb_fused_deep_moe path, use --with_w4a8 to select the mode.
#
...
def convert_nd_to_nz(x: torch.Tensor) -> torch.Tensor:
    """
    软件 NZ 重排：把 ND 张量内的数据按 FRACTAL_NZ 的访存顺序重排，但张量本身
    保持普通 ND 格式（format 元数据不变）。用于 W4A8：op_host 要求 INT32 权重
    声明为 FORMAT_ND，但 kernel 读取时按 NZ 分形序，所以用纯 reshape/permute
    重排数据，而不是 npu_format_cast（那会把 format 元数据改成 FRACTAL_NZ，
    触发类型不匹配报错）。
    """
    fractal_size_dict = {
        torch.int8: (16, 32),
...
class DecodeMoeOps(torch.nn.Module):
    ...
    def _checkout_datas(self, weight_datas, share_weight_datas):
        ...
        share_mm1_weight, share_mm1_weight_scale, share_mm2_weight, share_mm2_weight_scale, share_smooth_scales, share_gmm1_bias, share_gmm2_bias = share_weight_datas
    ...
    def _process_share_weights_after_loading(self, share_weight_datas):
        ...
        if self.with_w4a8:
            # W4A8: INT32 打包的 INT4 权重，op_host 要求 ND 格式，不能 npu_format_cast
            # 成 FRACTAL_NZ（会类型不匹配）；kernel 按 NZ 分形序读数据，用软件 NZ 重排。
            share_gmm1_weight = convert_tensor_into_parameter(
                convert_nd_to_nz(share_gmm1_weight) if share_gmm1_weight is not None else None)
            share_gmm2_weight = convert_tensor_into_parameter(
                convert_nd_to_nz(share_gmm2_weight) if share_gmm2_weight is not None else None)
        else:
            # W8A8: INT8 权重，op_host 要求 FRACTAL_NZ 格式
            share_gmm1_weight = convert_tensor_into_parameter(share_gmm1_weight, trans_nz=True)
            share_gmm2_weight = convert_tensor_into_parameter(share_gmm2_weight, trans_nz=True)
        self.share_gmm1_weight = share_gmm1_weight
        self.share_gmm1_weight_scale = convert_tensor_into_parameter(share_gmm1_weight_scale)
        self.share_gmm2_weight = share_gmm2_weight
...
class SmallOps(DecodeMoeOps):
    """参考实现：
    - W8A8: 组合小算子 (dispatch + gmm1 + swiglu + gmm2 + combine) 在 NPU 上计算。
    - W4A8: dispatch/combine 走 NPU，GMM1/GMM2 部分用 TorchSimulator 在 CPU 上模拟。
    """
    ...
    def __init__(self,
                 ...
                 share_weight_datas,
                 with_w4a8=False):
        super().__init__(ep_hcomm_info, meta_info, weight_datas, share_weight_datas, with_w4a8)
    ...
    def _process_weights_after_loading(self, weight_datas):
        gmm1_weight, gmm1_weight_scale, gmm2_weight, gmm2_weight_scale, smooth_scales, gmm1_bias, gmm2_bias = weight_datas
        if self.with_w4a8:
            # W4A8: TorchSimulator 需要 ND/CPU 权重
    ...
    def _apply_ops(self, x, expert_ids, expert_scales, x_active_mask):
        if self.with_share:
            ...
            if self.with_w4a8:
                # W4A8 share 场景只返回量化后的 x，供 run_once 做 CPU 模拟校验
                return share_output
        else:
            share_output = None
        ...
        if self.with_w4a8:
            # ---- W4A8: GMM1/GMM2 用 TorchSimulator 在 CPU 模拟 ----
            weight_int4 = TorchSimulator._int32_unpack_int4(self.gmm1_weight)
            total_tokens = sum(expert_token_nums)
            x_sim = expand_x.cpu()[:total_tokens]
            x_high, x_low = TorchSimulator._a8w4_pre_process(x_sim)
...
class ShmemFusionOp(DecodeMoeOps):
    ...
    def _init_shmem_buffer(self):
        # master 侧接口: umdk_cam_op_lib.ZbBuffer(rank, num_ranks, local_mem_size, ip_port,
        #                                           hidden, num_experts, use_quant, global_bs)
        # hidden 是 token 的隐藏维度 K。直接从参数取原始 token_hidden_size，
        # 不能从权重 shape 推导：W8A8 权重转 FRACTAL_NZ 后维度从 [E,K,N] 变成
        # [E,N/64,K/16,16,64]，shape 已被格式转换改写，推导会得到错误值。
        hidden_size = self.token_hidden_size
        self.buffer = umdk_cam_op_lib.ZbBuffer(
            self.global_rank_id,
            self.ep_world_size,
            self.shmem_memsize,
            self.ip_port,
    ...
    def _process_share_weights_after_loading(self, share_weight_datas):
        ...
        _, _, _, _, share_smooth_scales, _, _ = share_weight_datas
    ...
    def _process_weights_after_loading(self, weight_datas):
        ...
        if self.with_w4a8:
            # W4A8: INT32 打包的 INT4 权重，op_host 要求 ND 格式，不能 npu_format_cast
            # 成 FRACTAL_NZ（会类型不匹配）；kernel 按 NZ 分形序读数据，用软件 NZ 重排。
            gmm1_weight = convert_tensor_into_parameter(convert_nd_to_nz(gmm1_weight))
            gmm2_weight = convert_tensor_into_parameter(convert_nd_to_nz(gmm2_weight))
        else:
            # W8A8: INT8 权重，op_host 要求 FRACTAL_NZ 格式
        ...
        gmm1_bias_param = convert_tensor_into_parameter(gmm1_bias)
        gmm2_bias_param = convert_tensor_into_parameter(gmm2_bias)
        if self.dynamic_eplb:
            ...
            self.gmm1_bias = [
                bias.clone() for bias in gmm1_bias_param.unbind(dim=0)
            ] if gmm1_bias_param is not None else None
            self.gmm2_bias = [
                bias.clone() for bias in gmm2_bias_param.unbind(dim=0)
            ] if gmm2_bias_param is not None else None
        else:
            ...
            self.gmm1_bias = [gmm1_bias_param.clone()] if gmm1_bias_param is not None else None
            self.gmm2_bias = [gmm2_bias_param.clone()] if gmm2_bias_param is not None else None
        self.smooth_scales = convert_tensor_into_parameter(smooth_scales)
    ...
    def _apply_ops(self, x, expert_ids, expert_scales, x_active_mask):
        if self.with_w4a8:
            ...
            # W8A8: 无 bias，传空的 TensorList 满足接口签名
            empty_bias = torch.empty(0, dtype=torch.float32, device=x.device)
            gmm1_bias = [empty_bias]
            gmm2_bias = [empty_bias]
            share_gmm1_bias = None
            share_gmm2_bias = None
        # fused_deep_moe 入参（与 zb_buffer.h 最新签名一致）：
...
def generate_datas(batch_size,
    ...
    if with_w4a8:
        # W4A8: INT32 打包的 INT4 权重 + 反量化 bias
        ...
        # W8A8: INT8 权重 + per-channel scale，无 bias
        gmm1_weight = torch.randint(
            -16, 16,
...
def run_once(local_rank_id,
    ...
    small_ops = SmallOps(ep_hcomm_info_small, meta_info, weight_datas_npu, share_weight_datas_npu, with_w4a8).npu()
    shmem_fused_ops = ShmemFusionOp(
        ep_hcomm_info_shmem, meta_info, weight_datas_npu, share_weight_datas_npu,
    ...
    # 仅测试 zb_buffer.zb_fused_deep_moe（SHMEM ZeroBuffer 路径）
    shmem_fused_op_output = shmem_fused_ops(*input_datas)
    fused_op_token_output, fused_op_share_output, fused_op_count_output = shmem_fused_op_output
    torch_npu.npu.synchronize(device_id)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--with_w4a8", action="store_true", default=False,
                        help="use W4A8 quantization mode (default: W8A8)")
    ...
    BASE_KWARGS["with_w4a8"] = args.with_w4a8
