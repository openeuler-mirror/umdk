...
static int run_test(test_context_t *test_ctx)
{
    ...
    if (ctx->app_id == PROC_2) {
        ...
        for (int i = 0; i < WR_NUM; i++) {
            memset(ctx->l_ctx.seg_cfg[i].va, 'b', (i + 1) * length);
        }
    } else {
        for (int i = 0; i < WR_NUM; i++) {
            memset(ctx->l_ctx.seg_cfg[i].va, 'a', (i + 1) * length);
        }
    }
    ...
    if (ctx->app_id == PROC_1) {
        for (int i = 0; i < WR_NUM; i++) {
            ...
            jfs_wr[i]->rw.src.sge[0].addr = ctx->r_ctx[PROC_2 - 1].tseg[i]->seg.ubva.va;
            jfs_wr[i]->rw.src.sge[0].len = (i + 1) * length;
            jfs_wr[i]->rw.src.sge[0].tseg = ctx->r_ctx[PROC_2 - 1].tseg[i];
            
            // 读到本端地址
            jfs_wr[i]->rw.dst.sge = calloc(jfs_wr[i]->rw.dst.num_sge, sizeof(urma_sge_t));
            CHECK_JUMP(jfs_wr[i]->rw.dst.sge == NULL, EXIT, "calloc err!\n");
            jfs_wr[i]->rw.dst.sge[0].addr = ctx->l_ctx.seg_cfg[i].va;
            jfs_wr[i]->rw.dst.sge[0].len = (i + 1) * length;
            jfs_wr[i]->rw.dst.sge[0].tseg = ctx->l_ctx.tseg[i];
        }

        // before read seg is 'a'
        for (int i = 0; i < WR_NUM; i++) {
            TEST_LOG_INFO("--- before msg:%s\n", (char *)jfs_wr[i]->rw.dst.sge[0].addr);
        }
    }
