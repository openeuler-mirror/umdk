...
|expert_ids|Tensor|Required|Shape:(batch_size, topk)，int32 type, range: [-INF, num_experts)，where negative numbers indicate no dispatch, and values exceeding num_experts will cause an exception. A token cannot be sent to an expert more than once.|target expert IDs of each token|
