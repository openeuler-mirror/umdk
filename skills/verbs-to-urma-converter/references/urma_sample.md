# URMA 完整工作实例：urma_sample.c
> 来源： umdk/src/urma/examples/urma_sample.c 
> 这是 URMA 客户端/服务器应用的规范参考实现，演示了 RC/RM/UM 传输模式、事件驱动、RDMA读写、Send/Recv、多客户端支持，以及客户端与服务器在同一进程中共存
```c
/*
...
### 1. 完整的资源生命周期
- `urma_init()` -> 设备查询 -> `urma_create_context` -> JFEC -> JFC -> JFR -> Jetty -> 注册段
- `uninit_context` 中按相反顺序清理

### 2. 通过 Socket 交换地址
- 将 EID + UASID + seg VA + Jetty ID 打包到结构体
- 通过 TCP socket 交换
- 对端解包

### 3. RC 模式的 Import + Bind
- 先调用 `urma_import_jetty()`
- 仅 RC 模式需调用 `urma_bind_jetty()`
- 必须设置 `rjetty.tp_type`

### 4. RDMA 写/读操作
- 写：本地源 -> 远端目标
- 读：交换 src/dst （远端源 -> 本地目标）

### 5. 带缓冲区回收的 Send/Recv
- 预投递接收缓冲区
- 发送消息
- 轮训发送和接收完成
- 通过 `cr.flag.bs.s_r` 区分发送与接收完成

### 6. 事件驱动模式
- `urma_wait_jfc()` -> `urma_poll_jfc()` -> `urma_ack_jfc()` -> `urma_rearm_jfc()`
- 轮询回退：带 sleep 的循环