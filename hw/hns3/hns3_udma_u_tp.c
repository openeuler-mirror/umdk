...
{
	struct hns3_udma_u_context *udma_ctx = to_hns3_udma_ctx(urma_ctx);
	off_t offset;
	...
	if (qp->dwqe_page == MAP_FAILED) {
		HNS3_UDMA_LOG_ERR("failed to mmap direct wqe page, QPN = %lu.\n",
			     qp->qp_num);
		return EINVAL;
	}
...
void munmap_dwqe(struct hns3_udma_qp *qp)
{
	if (qp->dwqe_page) {
		if (munmap(qp->dwqe_page, HNS3_UDMA_DWQE_PAGE_SIZE))
			HNS3_UDMA_LOG_ERR("failed to munmap direct wqe page, QPN = %lu.\n",
					  qp->qp_num);
		qp->dwqe_page = NULL;
	}
}
