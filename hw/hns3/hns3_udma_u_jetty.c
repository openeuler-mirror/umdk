...
urma_status_t verify_jfs_init_attr(urma_context_t *ctx, urma_jfs_cfg_t *cfg)
{
	...
	if (!cfg->depth || cfg->depth > udma_ctx->max_jfs_wr ||
		...
		HNS3_UDMA_LOG_ERR(
			"Invalid jfs cfg: sq depth: %u, max_jfs_wr:%u, sq max_sge: %u, udma_ctx->max_jfs_sge: %u.\n",
...
static urma_status_t alloc_jfr(struct hns3_udma_u_jetty *jetty, urma_context_t *ctx,
			       urma_jetty_cfg_t *jetty_cfg)
{
	...
	if (jetty->share_jfr) {
		jetty->hns3_udma_jfr = to_hns3_udma_jfr(jetty_cfg->shared.jfr);
	} else {
		if (!udma_ctx->dca_ctx.unit_size)
			urma_jfr = hns3_udma_u_create_jfr_rq(ctx, jetty_cfg->jfr_cfg, jetty);
		else
			urma_jfr = hns3_udma_u_create_jfr(ctx, jetty_cfg->jfr_cfg);
		if (!urma_jfr) {
			HNS3_UDMA_LOG_ERR("failed to create jfr.\n");
			return URMA_FAIL;
		}
		jetty->hns3_udma_jfr = to_hns3_udma_jfr(urma_jfr);
	}

	return URMA_SUCCESS;
}

static urma_status_t alloc_qp_node_table(struct hns3_udma_u_jetty *jetty,
					 ...
					 urma_jetty_cfg_t *jetty_cfg)
{
	...
	if (jetty->tp_mode == URMA_TM_UM) {
		if (jetty->hns3_udma_jfr == NULL)
			jetty->um_qp = hns3_udma_alloc_qp(udma_ctx, true,
							  &jetty_cfg->jfs_cfg,
							  jetty_cfg->jfr_cfg);
		else
			jetty->um_qp = hns3_udma_alloc_qp(udma_ctx, true,
		...
		if (!jetty->um_qp) {
			HNS3_UDMA_LOG_ERR("UM qp alloc failed, jetty_id = %u.\n",
		...
		if (!jetty->rc_node) {
			HNS3_UDMA_LOG_ERR("RC node alloc failed, jetty_id = %u.\n",
					  jetty->urma_jetty.jetty_id.id);
			return URMA_ENOMEM;
		}

		if (jetty->hns3_udma_jfr == NULL)
			jetty->rc_node->qp = hns3_udma_alloc_qp(udma_ctx, true,
								&jetty_cfg->jfs_cfg,
								jetty_cfg->jfr_cfg);
		else
			jetty->rc_node->qp = hns3_udma_alloc_qp(udma_ctx, true,
								&jetty_cfg->jfs_cfg, NULL);
		if (!jetty->rc_node->qp) {
			HNS3_UDMA_LOG_ERR("alloc rc_node failed.\n");
...
urma_status_t hns3_udma_add_to_qp_table(struct hns3_udma_u_context *ctx, struct hns3_udma_qp *qp,
					uint32_t qpn)
{
	struct hns3_udma_jfs_qp_node *qp_node;

	qp_node = (struct hns3_udma_jfs_qp_node *)calloc(1, sizeof(*qp_node));
	if (qp_node == NULL) {
		HNS3_UDMA_LOG_ERR("failed to calloc qp_node.\n");
		return URMA_ENOMEM;
	}
	qp_node->jfs_qp = qp;

	(void)pthread_rwlock_wrlock(&ctx->jfs_qp_table_lock);
	if (!hns3_udma_hmap_insert(&ctx->jfs_qp_table, &qp_node->node, qpn)) {
		HNS3_UDMA_LOG_ERR("failed insert qp_node into jfs_qp_table.\n");
		free(qp_node);
		qp_node = NULL;
...
void hns3_udma_remove_from_qp_table(struct hns3_udma_u_context *ctx, uint32_t qpn)
{
	struct hns3_udma_jfs_qp_node *qp_node;
	struct hns3_udma_hmap_node *node;

	node = hns3_udma_table_first_with_hash(&ctx->jfs_qp_table,
					  &ctx->jfs_qp_table_lock, qpn);
	if (node) {
		qp_node = to_hns3_udma_jfs_qp_node(node);
		(void)pthread_rwlock_wrlock(&ctx->jfs_qp_table_lock);
		hns3_udma_hmap_remove(&ctx->jfs_qp_table, node);
		(void)pthread_rwlock_unlock(&ctx->jfs_qp_table_lock);
		free(qp_node);
		return;
	}
	HNS3_UDMA_LOG_ERR("failed to find jetty qp.\n");
}
					   ...
					   urma_jetty_cfg_t *cfg)
{
	...
	if (!jetty->share_jfr) {
		cmd.jfr_id = jetty->hns3_udma_jfr->jfrn;
		cmd.srqn = jetty->hns3_udma_jfr->srqn;
	}
	...
	if (urma_cmd_create_jetty(ctx, &jetty->urma_jetty, cfg, &udata)) {
		HNS3_UDMA_LOG_ERR("urma cmd create jetty failed.\n");
		return URMA_ENOMEM;
	}
...
static void rc_free_node(struct hns3_udma_u_context *udma_ctx, struct hns3_udma_u_jetty *udma_jetty)
{
	struct hns3_udma_qp *qp = udma_jetty->rc_node->qp;

	if (udma_jetty->rc_node->tjetty)
		hns3_udma_u_unbind_jetty(&udma_jetty->urma_jetty);

	hns3_udma_free_sw_db(udma_ctx, qp->sdb, HNS3_UDMA_JETTY_TYPE_DB);

	if (qp->dca_wqe.bufs)
		free(qp->dca_wqe.bufs);
	else
		hns3_udma_free_buf(&qp->buf);
...
static void um_free_qp(struct hns3_udma_u_jetty *udma_jetty)
{
	struct hns3_udma_u_context *udma_ctx = to_hns3_udma_ctx(udma_jetty->urma_jetty.urma_ctx);

	hns3_udma_free_sw_db(udma_ctx, udma_jetty->um_qp->sdb, HNS3_UDMA_JETTY_TYPE_DB);
	hns3_udma_free_buf(&udma_jetty->um_qp->buf);
	free(udma_jetty->um_qp->sq.wrid);
	free(udma_jetty->um_qp);
}

static void delete_jetty_qp_node(struct hns3_udma_u_context *udma_ctx,
...
void delete_jetty_node(struct hns3_udma_u_context *udma_ctx, uint32_t id)
{
	...
	if (!--udma_ctx->jetty_table[table_id].refcnt) {
		...
		udma_ctx->jetty_table[table_id].table = NULL;
	} else {
...
urma_status_t insert_jetty_node(struct hns3_udma_u_context *udma_ctx,
...
urma_status_t verify_jetty_trans_mode(urma_jetty_cfg_t *jetty_cfg)
{
	...
	if (jfs_trans_mode != jfr_trans_mode) {
		HNS3_UDMA_LOG_ERR("jfs_trans_mode: %u is different from jfr_trans_mode: %u.\n",
			     jfs_trans_mode, jfr_trans_mode);
		return URMA_EINVAL;
	}

	if (jfs_trans_mode != URMA_TM_UM && jfs_trans_mode != URMA_TM_RC) {
		HNS3_UDMA_LOG_ERR("The jetty trans_mode(%d) is not supported.\n", jfs_trans_mode);
		return URMA_EINVAL;
	}
...
static inline void init_jetty_param(struct hns3_udma_u_jetty *udma_jetty,
				    urma_jetty_cfg_t *jetty_cfg)
{
...
static int exec_jetty_delete_cmd(struct hns3_udma_u_context *ctx, struct hns3_udma_u_jetty *jetty)
{
	struct hns3_udma_qp *qp = NULL;
...
static urma_jetty_t *hns3_udma_u_create_jetty_rq(urma_context_t *ctx,
						 urma_jetty_cfg_t *jetty_cfg)
{
	struct hns3_udma_u_context *udma_ctx = to_hns3_udma_ctx(ctx);
	struct hns3_udma_u_jetty *udma_jetty;
	...
	udma_jetty = (struct hns3_udma_u_jetty *)calloc(1, sizeof(*udma_jetty));
	if (!udma_jetty) {
		HNS3_UDMA_LOG_ERR("failed to alloc jetty.\n");
		return NULL;
	}

	ret = verify_jetty_trans_mode(jetty_cfg);
	if (ret) {
		HNS3_UDMA_LOG_ERR("failed verify jetty trans mode.\n");
	...
	if (ret) {
		HNS3_UDMA_LOG_ERR("exec jetty create cmd failed.\n");
...
err_exec_jetty_create_cmd:
	if (!udma_jetty->share_jfr)
		hns3_udma_u_delete_jfr(&udma_jetty->hns3_udma_jfr->urma_jfr);
...
urma_jetty_t *hns3_udma_u_create_jetty(urma_context_t *ctx,
				       urma_jetty_cfg_t *jetty_cfg)
{
	struct hns3_udma_u_context *udma_ctx = to_hns3_udma_ctx(ctx);
	struct hns3_udma_u_jetty *udma_jetty;
	...
	if (!jetty_cfg->flag.bs.share_jfr && udma_ctx->dca_ctx.unit_size == 0)
		return hns3_udma_u_create_jetty_rq(ctx, jetty_cfg);
...
urma_status_t hns3_udma_u_delete_jetty(urma_jetty_t *jetty)
{
	struct hns3_udma_u_context *udma_ctx = to_hns3_udma_ctx(jetty->urma_ctx);
	struct hns3_udma_u_jetty *udma_jetty = to_hns3_udma_jetty(jetty);
	int ret;
	...
	if (ret) {
		HNS3_UDMA_LOG_ERR("urma_cmd_delete_jetty failed, ret:%d.\n", ret);
		return URMA_FAIL;
	}
...
urma_target_jetty_t *hns3_udma_u_import_jetty(urma_context_t *ctx,
					      urma_rjetty_t *rjetty,
					      urma_token_t *rjetty_token)
{
	struct hns3_udma_u_target_jetty *udma_target_jetty;
	urma_cmd_udrv_priv_t udata = {};
	urma_target_jetty_t *tjetty;
	urma_tjetty_cfg_t cfg = {};
	int ret;

	udma_target_jetty = (struct hns3_udma_u_target_jetty *)
			    calloc(1, sizeof(struct hns3_udma_u_target_jetty));
	if (!udma_target_jetty) {
		HNS3_UDMA_LOG_ERR("target jetty alloc failed.\n");
		return NULL;
	}
	...
	if (tjetty->flag.bs.sub_trans_mode != URMA_SUB_TRANS_MODE_USER_TP) {
		...
		if (ret) {
			HNS3_UDMA_LOG_ERR("import jetty failed, ret = %d.\n", ret);
...
urma_status_t hns3_udma_u_unimport_jetty(urma_target_jetty_t *target_jetty)
{
	struct hns3_udma_u_target_jetty *udma_target_jetty = to_hns3_udma_target_jetty(target_jetty);
	int ret;

	if (udma_target_jetty->refcnt > 1) {
		HNS3_UDMA_LOG_ERR("the target jetty is still being used, id = %u.\n",
			     target_jetty->id.id);
		return URMA_FAIL;
	}
	...
	if (target_jetty->flag.bs.sub_trans_mode != URMA_SUB_TRANS_MODE_USER_TP) {
		...
		if (ret) {
			HNS3_UDMA_LOG_ERR("unimport jetty failed, ret = %d.\n", ret);
			return URMA_FAIL;
		}
	}
	free(udma_target_jetty);
...
static urma_status_t verify_jetty_bind(struct hns3_udma_u_jetty *udma_jetty)
{
	if (udma_jetty->tp_mode != URMA_TM_RC) {
		HNS3_UDMA_LOG_ERR("Invalid jetty type.\n");
		return URMA_EINVAL;
	}

	if (udma_jetty->rc_node == NULL) {
		HNS3_UDMA_LOG_ERR("RC node is invalid.\n");
		return URMA_EINVAL;
	}

	if (udma_jetty->rc_node->qp == NULL) {
		HNS3_UDMA_LOG_ERR("RC node qp is null.\n");
		return URMA_EINVAL;
	}
					 ...
					 struct hns3_udma_qp *qp)
{
	...
	if (ret) {
		HNS3_UDMA_LOG_ERR("urma cmd bind jetty failed, ret = %d.\n", ret);
		return URMA_FAIL;
	}
...
urma_status_t hns3_udma_u_bind_jetty(urma_jetty_t *jetty, urma_target_jetty_t *tjetty)
{
	struct hns3_udma_u_target_jetty *udma_target_jetty = to_hns3_udma_target_jetty(tjetty);
	struct hns3_udma_u_jetty *udma_jetty = to_hns3_udma_jetty(jetty);
	int ret;

	ret = verify_jetty_bind(udma_jetty);
	if (ret) {
		HNS3_UDMA_LOG_ERR("Invalid input parameters of bind_jetty.\n");
		return URMA_EINVAL;
	}

	if (udma_jetty->rc_node->tjetty == tjetty) {
		HNS3_UDMA_LOG_INFO("reentry bind jetty, jetty_id = %u, tjetty_id = %u.\n",
			      jetty->jetty_id.id, tjetty->id.id);
		return URMA_SUCCESS;
	}

	if (udma_jetty->rc_node->tjetty != NULL) {
		HNS3_UDMA_LOG_ERR("The jetty has already bind a remote jetty.\n");
		return URMA_EEXIST;
	}

	if (udma_jetty->sub_trans_mode != tjetty->flag.bs.sub_trans_mode) {
		HNS3_UDMA_LOG_ERR("The sub_trans_mode does not match.\n");
		return URMA_EINVAL;
	}
	...
	if (tjetty->flag.bs.sub_trans_mode != URMA_SUB_TRANS_MODE_USER_TP) {
		...
		if (ret) {
			HNS3_UDMA_LOG_ERR("exec jetty bind cmd failed.\n");
			return URMA_FAIL;
		}
	}
...
urma_status_t hns3_udma_u_unbind_jetty(urma_jetty_t *jetty)
{
	struct hns3_udma_u_jetty *udma_jetty = to_hns3_udma_jetty(jetty);
	struct hns3_udma_u_target_jetty *udma_target_jetty;
	urma_target_jetty_t *tjetty;
	...
	ret = verify_jetty_bind(udma_jetty);
	if (ret) {
		HNS3_UDMA_LOG_ERR("Invalid input parameters of unbind_jetty.\n");
		return URMA_EINVAL;
	}

	tjetty = udma_jetty->rc_node->tjetty;
	if (tjetty == NULL) {
		HNS3_UDMA_LOG_ERR("The jetty has not bind a remote jetty.\n");
		return URMA_FAIL;
	}
	...
	if (tjetty->flag.bs.sub_trans_mode != URMA_SUB_TRANS_MODE_USER_TP) {
		...
		if (ret) {
			HNS3_UDMA_LOG_ERR("urma_cmd_unbind_jetty failed.\n");
			return URMA_FAIL;
		}
	}

	udma_target_jetty = to_hns3_udma_target_jetty(tjetty);
...
static struct hns3_udma_qp *get_qp_for_tjetty(struct hns3_udma_u_jetty *udma_jetty,
					      urma_target_jetty_t *tjetty)
{
	struct hns3_udma_qp *udma_qp = NULL;

	if (udma_jetty->rc_node->tjetty == NULL) {
		HNS3_UDMA_LOG_ERR("The jetty not bind a remote jetty, jetty_id = %u.\n",
			     udma_jetty->urma_jetty.jetty_id.id);
		return NULL;
	}
...
struct hns3_udma_qp *get_qp_of_jetty(struct hns3_udma_u_jetty *udma_jetty,
				     urma_jfs_wr_t *wr)
{
	struct hns3_udma_qp *udma_qp = NULL;

	if (udma_jetty->tp_mode != URMA_TM_RC && !wr->tjetty) {
		HNS3_UDMA_LOG_ERR("Failed to get jetty qp, tjetty of wr is null.\n");
		return NULL;
	}
...
static urma_status_t hns3_udma_u_post_jetty_rc_wr(struct hns3_udma_u_context *udma_ctx,
						  struct hns3_udma_u_jetty *udma_jetty,
						  urma_jfs_wr_t *wr,
						  urma_jfs_wr_t **bad_wr)
{
	...
	for (nreq = 0; wr; ++nreq, wr = wr->next) {
		ret = hns3_udma_u_post_rcqp_wr(udma_ctx, udma_qp, wr, &wqe, nreq);
		if (ret) {
			*bad_wr = wr;
			break;
		}
	...
	if (likely(nreq)) {
		...
		*udma_qp->sdb = udma_qp->sq.head;
		hns3_udma_u_ring_sq_doorbell(udma_ctx, udma_qp, wqe, nreq);
	}
...
static urma_status_t hns3_udma_u_post_jetty_um_wr(struct hns3_udma_u_context *udma_ctx,
						  struct hns3_udma_u_jetty *udma_jetty,
						  urma_jfs_wr_t *wr,
						  urma_jfs_wr_t **bad_wr)
{
...
urma_status_t hns3_udma_u_post_jetty_send_wr(urma_jetty_t *jetty,
					     urma_jfs_wr_t *wr,
					     urma_jfs_wr_t **bad_wr)
{
	struct hns3_udma_u_context *udma_ctx = to_hns3_udma_ctx(jetty->urma_ctx);
	...
	if (udma_jetty->tp_mode == URMA_TM_RC)
		ret = hns3_udma_u_post_jetty_rc_wr(udma_ctx, udma_jetty, wr, bad_wr);
	else
		ret = hns3_udma_u_post_jetty_um_wr(udma_ctx, udma_jetty, wr, bad_wr);
...
urma_status_t hns3_udma_u_post_jetty_recv_wr(urma_jetty_t *jetty,
					     urma_jfr_wr_t *wr,
					     urma_jfr_wr_t **bad_wr)
{
	struct hns3_udma_u_context *ctx = to_hns3_udma_ctx(jetty->urma_ctx);
	struct hns3_udma_u_jetty *udma_jetty = to_hns3_udma_jetty(jetty);
	struct hns3_udma_u_jfr *udma_jfr = udma_jetty->hns3_udma_jfr;
	urma_status_t ret = URMA_SUCCESS;
	uint32_t nreq;
...
int hns3_udma_u_flush_jetty(urma_jetty_t *jetty, int cr_cnt, urma_cr_t *cr)
{
	struct hns3_udma_u_jetty *udma_jetty = to_hns3_udma_jetty(jetty);
	struct hns3_udma_qp *qp;
	int n_flushed = 0;
